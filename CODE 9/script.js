document.addEventListener('DOMContentLoaded', () => {
    ...
    const tracks = [
        { audio: document.getElementById('bg-music'), name: '💕 Romantic Melody' },
        { audio: document.getElementById('track-2'), name: '✨ Sweet Serenade' },
        { audio: document.getElementById('track-3'), name: '❤️ Love Theme' },
        { audio: document.getElementById('custom-track'), name: '🎵 Your Song', isCustom: true }
    ];
    ...
    if (customMusicInput) {
        customMusicInput.addEventListener('change', (e) => {
            ...
            if (file) {
                ...
                tracks[3].audio.src = url;
                tracks[3].name = `🎵 ${file.name.replace(/\.[^/.]+$/, "")}`;

                const customItem = document.querySelectorAll('.playlist-item')[3];
                if (customItem) {
                    customItem.querySelector('span').textContent = tracks[3].name;
                }

                switchTrack(3);
                if (playPauseBtn && !isPlaying) playPauseBtn.click();
    ...
    const moveNoButton = () => {
        ...
        const buffer = 20;
        const minX = cardRect.left + buffer;
        const maxX = cardRect.right - btnWidth - buffer;
        const minY = cardRect.top + buffer;
        const maxY = cardRect.bottom - btnHeight - buffer;
        ...
        } else if (noHoverCount >= 5) {
            noBtn.innerHTML = 'Are you sure? 🥺';
        } else if (noHoverCount >= 3) {
            noBtn.style.transform = 'scale(0.8)';
        }

        const yesScale = yesBtnOriginalSize + (noHoverCount * 0.05);
    ...
    // Phase 3: Secret Message Reveal
    const revealSecretBtn = document.getElementById('reveal-secret');
    const secretMessage = document.getElementById('secret-message');

    if (revealSecretBtn && secretMessage) {
        revealSecretBtn.addEventListener('click', () => {
            secretMessage.classList.toggle('revealed');
            playSound(clickSound);

            if (secretMessage.classList.contains('revealed')) {
                revealSecretBtn.textContent = '✨ Hide Secret ✨';
            } else {
                revealSecretBtn.textContent = '✨ Click for a Secret Message ✨';
            }
        });
    }
    ...
    // Phase 3: Reason Cards Reveal on Scroll
    ...
    // Phase 3: Easter Egg - Konami Code
    ...
    function activateEasterEgg() {
        ...
        (function frame() {
            confetti({
                ...
                spread: 55,
                origin: { x: 0 },
                colors: ['#ff4d6d', '#ff758f', '#ffd700', '#ff69b4', '#87ceeb']
            });
            confetti({
                particleCount: 7,
                angle: 120,
        ...
        alert('🎉 You found the secret! You\'re absolutely amazing! 🎉');
        playSound(confettiSound);
    }

    // Phase 3: Double-click Easter Egg on Hearts
    document.querySelectorAll('.heart, .main-emoji').forEach(el => {
